...
workdir="${ORION_DRIZZLE_SMOKE_DIR:-$(mktemp -d)}"
keep_workdir="${ORION_DRIZZLE_SMOKE_KEEP:-0}"
raft_port="${ORION_DRIZZLE_SMOKE_RAFT_PORT:-17401}"
http_port="${ORION_DRIZZLE_SMOKE_HTTP_PORT:-18391}"
database="${ORION_DRIZZLE_SMOKE_DATABASE:-drizzle_smoke}"
...
if ! kill -0 "$pid" 2>/dev/null; then
  echo "orion exited before Drizzle smoke could run" >&2
  tail -n 200 "$log_file" >&2 || true
  exit 1
fi
...
echo "running Drizzle libSQL smoke"
LIBSQL_URL="http://127.0.0.1:${http_port}/${database}" \
  "$repo_root/scripts/local-drizzle-libsql-smoke.sh"

echo "Drizzle libSQL smoke passed"
if [[ "$keep_workdir" == "1" ]]; then
