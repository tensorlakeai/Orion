...
workdir="${ORION_CLIENT_SMOKE_DIR:-$(mktemp -d)}"
keep_workdir="${ORION_CLIENT_SMOKE_KEEP:-0}"
raft_port="${ORION_CLIENT_SMOKE_RAFT_PORT:-17201}"
http_port="${ORION_CLIENT_SMOKE_HTTP_PORT:-18191}"
database="${ORION_CLIENT_SMOKE_DATABASE:-client_smoke}"
...
if ! kill -0 "$pid" 2>/dev/null; then
  echo "orion exited before client smoke could run" >&2
  tail -n 200 "$log_file" >&2 || true
  exit 1
fi
...
echo "running official @libsql/client smoke"
LIBSQL_URL="http://127.0.0.1:${http_port}/${database}" \
  "$repo_root/scripts/local-libsql-client-smoke.sh"

echo "official libSQL client smoke passed"
if [[ "$keep_workdir" == "1" ]]; then
