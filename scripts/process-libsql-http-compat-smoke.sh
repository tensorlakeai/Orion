...
workdir="${ORION_HTTP_COMPAT_SMOKE_DIR:-$(mktemp -d)}"
keep_workdir="${ORION_HTTP_COMPAT_SMOKE_KEEP:-0}"
raft_port="${ORION_HTTP_COMPAT_SMOKE_RAFT_PORT:-17501}"
http_port="${ORION_HTTP_COMPAT_SMOKE_HTTP_PORT:-18491}"
database="${ORION_HTTP_COMPAT_SMOKE_DATABASE:-http_compat_smoke}"
...
if ! kill -0 "$pid" 2>/dev/null; then
  echo "orion exited before HTTP compatibility smoke could run" >&2
  tail -n 200 "$log_file" >&2 || true
  exit 1
fi
...
echo "running raw libSQL HTTP compatibility smoke"
node "$repo_root/scripts/libsql-http-compat-smoke.mjs" \
  "http://127.0.0.1:${http_port}/${database}"

echo "raw libSQL HTTP compatibility smoke passed"
if [[ "$keep_workdir" == "1" ]]; then
