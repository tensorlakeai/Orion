...
workdir="${ORION_SQLLOGICTEST_SMOKE_DIR:-$(mktemp -d)}"
keep_workdir="${ORION_SQLLOGICTEST_SMOKE_KEEP:-0}"
raft_port="${ORION_SQLLOGICTEST_SMOKE_RAFT_PORT:-17301}"
http_port="${ORION_SQLLOGICTEST_SMOKE_HTTP_PORT:-18291}"
database="${ORION_SQLLOGICTEST_SMOKE_DATABASE:-sqllogictest}"
...
if ! kill -0 "$pid" 2>/dev/null; then
  echo "orion exited before sqllogictest could run" >&2
  tail -n 200 "$log_file" >&2 || true
  exit 1
fi
...
echo "running sqllogictest corpus"
node "$repo_root/scripts/orion-sqllogictest.mjs" \
  --url "http://127.0.0.1:${http_port}/${database}" \
  "$repo_root/testdata/sqllogictest/orion-core.slt"

echo "sqllogictest smoke passed"
if [[ "$keep_workdir" == "1" ]]; then
